...
export class EnhancedDipArbStrategy extends EventEmitter {
  ...
  private subscribeOrderbookBids(upId?: string, downId?: string): void {
    ...
    this.orderbookListener = (book: any) => {
    ...
    realtimeService.on('orderbook', this.orderbookListener);
    this.log('debug', 'Subscribed to orderbook events for bid data');
  }

  private unsubscribeOrderbookBids(): void {
    if (this.orderbookListener) {
      const dipArb = this.sdk.dipArb as any;
      const realtimeService = dipArb.realtimeService;
      if (realtimeService) {
        realtimeService.off('orderbook', this.orderbookListener);
      }
